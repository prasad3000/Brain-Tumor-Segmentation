...
for i in range(int(len(train_dir))):
    ...
    dir_ = 'G:\\my_study\\Fourth_year_project\\brain_tumor\\BraTS\Auto_focus_me\\data_preprocessing\\data_norm\\'
    
    direct_mask,_ = (dir_ + train_dir[5 * int(i/5)]).split("\n")
    direct_image,_ = train_dir[i].split("\n")    
    direct_mask = direct_mask + "\mask.nii.gz"
    direct_image_ = os.path.join(parent_dir +direct_image)
    
    pathToMainFolderWithSubjects = "BRATS2015_Training\\HGG\\"
