...
def saveImageToANewNiiWithHeaderFromOther(	normalizedImageNpArray,
    ...
    nib.save(newNormImg, outputFilepath + '.gz')
    print("Done.")
...
def do_normalization(windowTitle,
			...
			subjectsToProcess,
			channelToNormalizeFilepath, channelToNormalizeFilepath_, roiFilepath,			
			saveOutput,
			prefixToAddToOutp,			
			dtypeToSaveOutput,
	...
	if cutoffAtWholeImgMean :
        ...
        ### Save output ###
	if saveOutput:
		outputFilepath = 'BRATS2015_Training\\' + channelToNormalizeFilepath_
